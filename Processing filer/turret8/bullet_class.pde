class bullet {
  ...
  void paint() {
    ...
    case 0:                        // decaying bullet
      ...
      if (bulletCrit) {
        ...
        line(x, y, int(x-vx*100), int(y-vy*100));
        this.damage =10;
        this.v =65;
        this.weight= 10;
        strokeWeight(weight);
        ...
        line(x, y, int(x-vx*(tail-deathTimer*0.6)), int(y-vy*(tail-deathTimer*0.6)));
      }
      if (this.y > height) {    // bottom boundary bounce
        ...
        this.deathTimer=this.timeLimit;
      }
      break;

    case 1:                            // Big trailing rocket bullet
      strokeWeight(weight);
      stroke(random(100)+100, int(random(50)), 0);
      fill(random(100)+150, int( random(100)+150), 0);
      ...
      particles.add(new particle(0, color(100, 150, 0, 50), this.x, this.y, random(360), 5, int(random(30)+15), int(random(40)+10), 35)); // smoke Particles
      particles.add(new particle(4, color(100, 150, 0), this.x2, this.y2, int(this.bulletAngle), int(this.v-10), 30, 5, 8));                    // static particle
    ...
    case 2:                              // ------------------------------------------------direct laser bullet
      ...
      stroke(int(random(100)+150), int( random(50)), 0, 50);
    ...
    case 6:                        // -------------------------------------decay plasma balls
      ...
      fill(255);
      ellipse(x, y, int(10*(deathTimer-timeLimit)), int(10*(deathTimer-timeLimit)));
      stroke(255);
      strokeWeight(weight);
      // blendMode(NORMAL);
      break;
    ...
    case 10:                        // -------------------------------------mine
      ...
      if (activationTimer<activationTime) {
        ...
        line(x, y, int(x+cos(radians(rotation-5 ))*((activationTimer-activationTime)/4)), int(y+sin(radians(rotation-5 ))*((activationTimer-activationTime)/4)));

        text(activationTime-activationTimer, this.x, this.y-20); // RELOADING TIME
        activationTimer++;
      } else
      {
        if (bulletCrit==true) {
    ...
    case 11:                        // ---------------------------------------------------homing missles--------------------------------------------
      ...
      strokeWeight(weight);
      stroke(255, 0, 0);
      line( int(x-hvx*tail), int(y-hvy*tail), x, y);
      noStroke();
      fill(255, 100);
      ellipse(x, y, 20, 20);
    ...
    case 12:                                                // --------------------------------------------------- tesla  coil-------------------------------------
      ...
      showCrossheir=false;  // switch off crooshier
      showCooldown=false; // switch off crooshier
      ...
      if (enemies.size() >target && target!=(-1)) {  // if enemies is present
        ...
        line(mouseX, mouseY, enemies.get(target).x, enemies.get(target).y);
  ...
  void hit( int index) {
    ...
    case 3:                          // ---------------------------------------------plasma bomb
      ...
      particles.add(new particle(1, color(255), enemies.get(index).x, enemies.get(index).y, particleAngle+random(10)-5, random(15), 10, 100, 10)); // blast Particles
      fill(255);
      break;
    ...
    case 4:                          // ------------------------------------------freeze granade death
      ...
      for (int i=0; enemies.size () > i; i++) {
        if (area/2>dist(this.x, this.y, enemies.get(i).x, enemies.get(i).y)) {
          ...
          particles.add(new particle(2, color(0, 100, 255), enemies.get(i).x, enemies.get(i).y, 0, 0, 5, 100, 90)); // freeze Particles
        }
      }

      particles.add(new particle(1, color(0, 100, 255), this.x, this.y, 0, 0, 5, 1000, 5)); // blast Particles
      for (int i=0; i < 15; i++) {
        particles.add(new particle(2, color(0, 100, 255), this.x, this.y, random(360), random(20), 5, int(random(50)+25), 90)); // freeze Particles
      }
    ...
    case 5:                          // --------------------------------------------regular sniper bullet death
      ...
      for (int d= 0; d < 4; d++) {
        bullets.add(new bullet(6, enemies.get(index).x, enemies.get(index).y, this.bulletAngle + random(180)-90, int( random(20)), 30, 30, 10, 0.1, false));  // ball effect burst
      }      
      for (int d= 0; d < 2; d++) {
        bullets.add(new bullet(6, enemies.get(index).x, enemies.get(index).y, this.bulletAngle + random(40)-20, int( random(120)), 30, 30, 10, 0.1, false));  // ball effect burst
      }
  ...
  void removeEBullets() {
    ...
    while (eBullets.size () > maxEBullets) {
      eBullets.remove(this);
      break;
    }
  }
  ...
  void displayInfoHoming(int i, int range, float X, float Y) {
    if (cheatEnabled) {
      fill(100);
      text("target " + i, X, int(Y-15));
      noFill();
      strokeWeight(1);
      stroke(255, 0, 0);
      line(X, Y, int(enemies.get(i).x), int(enemies.get(i).y));
      ellipse(X, Y, int(range), int(range));
    }
  }
