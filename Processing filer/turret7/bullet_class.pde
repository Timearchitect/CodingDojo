class bullet {
  ...
  float hitLenth=10, damage, force=1, weight=5, bulletAngle, hvx, hvy, homX=mouseX, homY=mouseY, senseRange;
  ...
  void paint() {
    ...
    case 4:                                      //-------------------------- freeze bounce grenade
      ...
      strokeWeight(random(10));
      line(x, y, x+vx*tail*v, y+vy*tail*v);
      particles.add(new particle(2, color(0, 100, 255), this.x, this.y, 0, 0, 0, int(random(10)+5), 15)); // freeze Particles
      ...
      if (this.y > height) {    // bottom boundary bounce
        vy*=-1;
        for (int i=0; i < 5; i++) {
          particles.add(new particle(2, color(0, 100, 255), this.x, this.y, random(360), random(15)+5, 5, int(random(10)+5), 20)); // freeze Particles
        }
      }
      if (this.x < 0) {  // left boundary bounce
        vx*=-1;
      ...
      if (this.x > width) {  // right boundary bounce
        vx*=-1;
        for (int i=0; i < 5; i++) {
          particles.add(new particle(2, color(0, 100, 255), this.x, this.y, random(360), random(15)+5, 5, int(random(10)+5), 20)); // freeze Particles
        }
      ...
      //line(this.x, this.y, this.x+cos(radians(this.bulletAngle))*50, this.y+sin(radians(this.bulletAngle))*50);
      break;
    ...
    case 8:                        // ------------------------------------------turret-------------------------------
      ...
      if (activationTimer> activationTime - cooldownStat) {
        bullets.add(new bullet(0, this.x+turretBarrelX, this.y+turretBarrelY, bulletAngle+random(8)-4, 30, 30, 5, 50, 0.1, false));
        activationTimer=0;
      }
      activationTimer++;
      strokeCap(SQUARE);
    ...
    case 9:                        // ---------------------------------------mouse aiming flameballs
      ...
      x+= (mouseX -x)*0.04;
      y+= (mouseY -y)*0.04;
      particles.add(new particle(8, color(255, random(100)+70, 70), this.x+random(50)-25, this.y+random(50)-25, this.bulletAngle, random(3), 50, int(random(30)), int(random(50)+100))); // triangle Particles
      //background(225, 150, 0);
      backgroundFadeColor=80;
      stroke(255);
      strokeWeight(weight);
      break;
    ...
    case 11:                        // ---------------------------------------------------homing missles
      ...
      particles.add(new particle(0, color(255), this.x, this.y, 0, 0, 5, int(random(30)+15), 20)); // smoke Particles
    ...
    case 12:                                                // --------------------------------------------------- gatling tesla
      ...
      if (enemies.size() >target && target!=(-1)) {  // if enemies is present
        ...
        enemies.get(target).force(random(360), this.v/20 );   // force enemy back
  ...
  void hit( int index) {
    ...
    case 3:                          // ---------------------------------------------plasma bomb
      ...
      particles.add(new particle(1, color(255), enemies.get(index).x, enemies.get(index).y, particleAngle, 10, 5, 100, 10)); // blast Particles
      fill(255);
      break;
    ...
    case 5:                          // --------------------------------------------regular sniper bullet death
      ...
      for (int d= 0; d < 4; d++) {
        bullets.add(new bullet(6, enemies.get(index).x, enemies.get(index).y, this.bulletAngle + random(180)-90, int( random(20)), 50, 30, 10, 1, false));  // ball effect burst
      }      
      for (int d= 0; d < 2; d++) {
        bullets.add(new bullet(6, enemies.get(index).x, enemies.get(index).y, this.bulletAngle + random(40)-20, int( random(120)), 50, 30, 10, 1, false));  // ball effect burst
      }
      bullets.add(new bullet(2, enemies.get(index).x+this.vx*2, enemies.get(index).y+this.vy*2, this.bulletAngle+90, 30, 260, 30, 600, 4, false));  // laser burst 90 deg
      bullets.add(new bullet(2, enemies.get(index).x, enemies.get(index).y, this.bulletAngle-90, 30, 260, 30, 600, 4, false));  // laser burst 90 deg
  ...
  void removeBullets() {              //  -----------------FISSILE deathTimer end--------------------------
    ...
    if (deathTimer > timeLimit) {
      ...
      if (type==4) {                      //-------------------------------- freeze granade
        ...
        for (int i=0; i < 20; i++) {
          particles.add(new particle(2, color(0, 100, 255), this.x, this.y, this.bulletAngle+ random(50)-25, random(15)+this.v*0.5, 5, int(random(50)+25), 100)); // freeze Particles
        }
      }
